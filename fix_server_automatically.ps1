...
# Использование: .\fix_server_automatically.ps1
...
$envCommand = "cd ${AppDir} && if [ ! -f .env ]; then cat > .env << 'EOF'
$envContent
EOF
fi"
Invoke-SSHCommand $envCommand "Создание .env файла"

# 5. Создание ecosystem.config.js если отсутствует
$ecosystemContent = @"
module.exports = {
  apps: [{
    name: 'nodeon-pyramid',
...
$ecosystemCommand = "cd ${AppDir} && if [ ! -f ecosystem.config.js ]; then cat > ecosystem.config.js << 'EOF'
$ecosystemContent
EOF
fi"
Invoke-SSHCommand $ecosystemCommand "Создание ecosystem.config.js"

# 6. Создание папки public если отсутствует
Invoke-SSHCommand "cd ${AppDir} && mkdir -p public" "Создание папки public"

# 7. Копирование index.html в public если отсутствует
...
# 13. Создание правильной конфигурации Nginx
$nginxConfig = @"
server {
    listen 80;
...
$nginxCommand = "cat > /etc/nginx/sites-available/nodeon << 'EOF'
$nginxConfig
EOF"

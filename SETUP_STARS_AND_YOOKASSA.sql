-- Настройка Stars и ЮКассы для NodeOn Pyramid
...
-- Таблица для Telegram Stars платежей
...
-- Таблица для ЮКасса платежей
CREATE TABLE IF NOT EXISTS nodeon_yookassa_payments (
    id SERIAL PRIMARY KEY,
    user_id INTEGER NOT NULL REFERENCES nodeon_users(id) ON DELETE CASCADE,
    payment_id VARCHAR(255) UNIQUE NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_yookassa_payments_user_id ON nodeon_yookassa_payments(user_id);
CREATE INDEX IF NOT EXISTS idx_yookassa_payments_payment_id ON nodeon_yookassa_payments(payment_id);
CREATE INDEX IF NOT EXISTS idx_yookassa_payments_status ON nodeon_yookassa_payments(status);
...
-- 4. Добавляем конфигурацию ЮКассы
...
-- 5. Создаем функции для работы с рефералами
...
-- 6. Создаем функцию для получения рефералов
...
-- 7. Создаем функцию для валидации реферальных ссылок
...
-- 10. Создаем разработчика (если не существует)
...
-- 11. Создаем статистику рефералов для разработчика

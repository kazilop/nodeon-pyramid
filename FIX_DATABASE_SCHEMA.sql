...
DROP TABLE IF EXISTS nodeon_stars_payments CASCADE;
DROP TABLE IF EXISTS nodeon_payment_requests CASCADE;
DROP TABLE IF EXISTS nodeon_referral_stats CASCADE;
DROP TABLE IF EXISTS nodeon_transactions CASCADE;
DROP TABLE IF EXISTS nodeon_users CASCADE;
DROP TABLE IF EXISTS nodeon_stars_config CASCADE;
DROP TABLE IF EXISTS nodeon_yookassa_config CASCADE;

-- 2. Создаем таблицы с правильной структурой
...
CREATE TABLE nodeon_transactions (
    id SERIAL PRIMARY KEY,
...
CREATE TABLE nodeon_referral_stats (
    id SERIAL PRIMARY KEY,
    user_id INTEGER NOT NULL REFERENCES nodeon_users(id) ON DELETE CASCADE,
...
CREATE TABLE nodeon_payment_requests (
    id SERIAL PRIMARY KEY,
    user_id INTEGER NOT NULL REFERENCES nodeon_users(id) ON DELETE CASCADE,
...
CREATE TABLE nodeon_stars_payments (
    id SERIAL PRIMARY KEY,
    user_id INTEGER NOT NULL REFERENCES nodeon_users(id) ON DELETE CASCADE,
...
CREATE TABLE nodeon_yookassa_payments (
    id SERIAL PRIMARY KEY,
    user_id INTEGER NOT NULL REFERENCES nodeon_users(id) ON DELETE CASCADE,
    payment_id VARCHAR(255) UNIQUE NOT NULL,
...
CREATE TABLE nodeon_yookassa_config (
    id SERIAL PRIMARY KEY,
    config_key VARCHAR(100) UNIQUE NOT NULL,
    config_value TEXT NOT NULL,
    description TEXT,
...
-- 3. Создаем индексы
CREATE INDEX idx_nodeon_users_telegram_id ON nodeon_users(telegram_id);
CREATE INDEX idx_nodeon_users_inviter_id ON nodeon_users(inviter_id);
CREATE INDEX idx_nodeon_users_created_at ON nodeon_users(created_at);
CREATE INDEX idx_nodeon_transactions_user_id ON nodeon_transactions(user_id);
CREATE INDEX idx_nodeon_transactions_created_at ON nodeon_transactions(created_at);
CREATE INDEX idx_nodeon_referral_stats_user_id ON nodeon_referral_stats(user_id);
CREATE INDEX idx_stars_payments_user_id ON nodeon_stars_payments(user_id);
CREATE INDEX idx_stars_payments_payment_id ON nodeon_stars_payments(payment_id);
CREATE INDEX idx_stars_payments_status ON nodeon_stars_payments(status);
CREATE INDEX idx_yookassa_payments_user_id ON nodeon_yookassa_payments(user_id);
CREATE INDEX idx_yookassa_payments_payment_id ON nodeon_yookassa_payments(payment_id);
CREATE INDEX idx_yookassa_payments_status ON nodeon_yookassa_payments(status);

-- 4. Добавляем конфигурацию Stars
...
-- 5. Добавляем конфигурацию ЮКассы
INSERT INTO nodeon_yookassa_config (config_key, config_value, description) VALUES
('rub_to_ndn_rate', '10.0', 'Курс обмена: 1 RUB = 10 NDN'),
('min_rub_purchase', '100', 'Минимальная покупка в рублях'),
('max_rub_purchase', '50000', 'Максимальная покупка в рублях'),
('yookassa_shop_id', 'YOUR_SHOP_ID', 'ID магазина в ЮКассе'),
('yookassa_secret_key', 'YOUR_SECRET_KEY', 'Секретный ключ ЮКассы'),
('yookassa_webhook_url', 'https://nodeon-production.up.railway.app/api/yookassa/webhook', 'Webhook URL для уведомлений'),
('yookassa_success_url', 'https://kazilop.github.io/nodeon-pyramid/?payment=success', 'URL успешной оплаты'),
('yookassa_cancel_url', 'https://kazilop.github.io/nodeon-pyramid/?payment=cancel', 'URL отмены оплаты');

-- 6. Создаем функции для работы с рефералами
...
-- 7. Создаем функцию для получения рефералов
...
-- 8. Создаем функцию для валидации реферальных ссылок
...
-- 10. Создаем функции для ЮКассы
CREATE OR REPLACE FUNCTION get_yookassa_config()
RETURNS JSON AS $$
DECLARE
    config_record RECORD;
    result JSON := '{}';
BEGIN
    FOR config_record IN 
        SELECT config_key, config_value FROM nodeon_yookassa_config
    LOOP
        result := result || json_build_object(config_record.config_key, config_record.config_value);
    END LOOP;
...
CREATE OR REPLACE FUNCTION create_yookassa_payment(
    p_user_id INTEGER,
    p_payment_id VARCHAR(255),
    p_amount_rub DECIMAL(10,2),
    p_ndn_amount DECIMAL(10,2)
)
RETURNS JSON AS $$
DECLARE
...
BEGIN
    ...
    INSERT INTO nodeon_yookassa_payments (
        user_id, payment_id, yookassa_payment_id, amount_rub, ndn_amount, status
    ) VALUES (
        p_user_id, p_payment_id, p_payment_id, p_amount_rub, p_ndn_amount, 'pending'
    );
...
CREATE OR REPLACE FUNCTION process_yookassa_payment(
    p_yookassa_payment_id VARCHAR(255),
    p_status VARCHAR(50)
)
RETURNS JSON AS $$
DECLARE
    ...
    result JSON;
BEGIN
    SELECT * INTO payment_record FROM nodeon_yookassa_payments 
    ...
    UPDATE nodeon_yookassa_payments SET
        status = p_status,
        updated_at = NOW(),
    ...
    IF p_status = 'succeeded' THEN
        UPDATE nodeon_users SET
            balance_ndn = balance_ndn + payment_record.ndn_amount,
            updated_at = NOW()
        ...
        INSERT INTO nodeon_transactions (
            ...
            payment_record.user_id, 'yookassa_purchase', payment_record.ndn_amount,
            'Покупка NDN через ЮКассу (' || payment_record.amount_rub || ' RUB)', 
            NOW()
        );
    END IF;
...
-- 11. Создаем разработчика (если не существует)
...
-- 12. Создаем статистику рефералов для разработчика

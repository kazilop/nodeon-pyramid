-- Создание серверного кошелька с 1 миллиардом NDN монет (ИСПРАВЛЕННАЯ ВЕРСИЯ)
...
-- 3. Удаляем и пересоздаем таблицу конфигурации Stars
...
-- 4. Создаем таблицу Stars платежей
CREATE TABLE IF NOT EXISTS nodeon_stars_payments (
    id SERIAL PRIMARY KEY,
    user_id BIGINT REFERENCES nodeon_users(telegram_id) ON DELETE CASCADE,
    payment_id VARCHAR(255) UNIQUE, -- ID транзакции в Stars
    stars_amount INT NOT NULL,
    ndn_amount NUMERIC(18, 2) NOT NULL,
    status VARCHAR(50) DEFAULT 'pending' NOT NULL, -- 'pending', 'completed', 'failed'
    wallet_transaction_id INTEGER REFERENCES nodeon_wallet_transactions(id),
    server_ndn_balance_before DECIMAL(18,2),
    server_ndn_balance_after DECIMAL(18,2),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    completed_at TIMESTAMP WITH TIME ZONE
);

-- 5. Добавляем колонки Stars в таблицу пользователей (если их нет)
...
-- 6. Создаем функции для работы с серверным кошельком
...
-- 7. Функция для получения конфигурации Stars
CREATE OR REPLACE FUNCTION get_stars_config()
RETURNS JSON AS $$
DECLARE
    config_record RECORD;
BEGIN
    SELECT * INTO config_record FROM nodeon_stars_config WHERE is_active = TRUE LIMIT 1;
    
    IF config_record IS NULL THEN
        RETURN '{"error": "Stars configuration not found"}'::JSON;
    END IF;
    ...
    RETURN json_build_object(
        ...
        'config', json_build_object(
            'id', config_record.id,
            'stars_to_ndn_rate', config_record.stars_to_ndn_rate,
            'ndn_to_stars_rate', config_record.ndn_to_stars_rate,
            'min_stars_purchase', config_record.min_stars_purchase,
            'max_stars_purchase', config_record.max_stars_purchase,
            'stars_withdrawal_fee_percent', config_record.stars_withdrawal_fee_percent,
...
-- 8. Функция для обработки покупки NDN за Stars
...
BEGIN
    ...
    -- Записываем платежный запрос (если есть payment_id)
    IF p_payment_id IS NOT NULL THEN
        INSERT INTO nodeon_stars_payments (
            user_id, payment_id, stars_amount, ndn_amount, 
            wallet_transaction_id, server_ndn_balance_before, server_ndn_balance_after,
            status
        ) VALUES (
            p_user_id, p_payment_id, p_stars_amount, p_ndn_amount,
            v_transaction_id, v_wallet_balance, v_new_balance,
            'completed'
        );
    END IF;
...
-- 9. Функция для обработки вывода NDN в Stars
...
BEGIN
    ...
    SET 
        balance_ndn = v_new_user_balance,
        balance_stars = balance_stars - p_stars_amount,
        total_stars_spent = total_stars_spent + p_stars_amount
    WHERE id = p_user_id;
...
-- 10. Функция для получения истории транзакций кошелька
...
-- 11. Инициализируем серверный кошелек с 1 миллиардом NDN
INSERT INTO nodeon_server_wallet (wallet_name, balance_ndn, total_ndn_issued, total_ndn_received)
VALUES ('main_wallet', 1000000000.00, 0.00, 0.00)
...
-- 12. Инициализируем конфигурацию Stars
...
-- 13. Создаем индексы для оптимизации
...
-- 14. Создаем представление для статистики кошелька
...
-- Готово! Серверный кошелек с 1 миллиардом NDN создан
SELECT 'Server wallet with 1 billion NDN created successfully!' as message;

...
## ✅ Адаптировано для папки /root/nodeon на Timeweb VPS!

Все конфигурации обновлены для работы в папке `/root/nodeon` на сервере.

## 📦 Файлы для загрузки

- **`nodeon-pyramid-root.tar.gz`** - архив для папки /root/nodeon
- **`ROOT_NODEON_DEPLOYMENT.md`** - подробные инструкции
...
scp nodeon-pyramid-root.tar.gz root@147.45.110.220:/root/

# Распаковка в нашу папку
cd /root/nodeon
tar -xzf /root/nodeon-pyramid-root.tar.gz
```
...
- ✅ **Папка в /root** - удобное расположение для root пользователя
...
**Файл для загрузки:** `nodeon-pyramid-root.tar.gz`

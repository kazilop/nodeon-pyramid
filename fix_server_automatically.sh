...
# Использование: ./fix_server_automatically.sh
...
run_ssh_command "cd $APP_DIR && if [ ! -f .env ]; then cat > .env << 'EOF'
NODE_ENV=production
PORT=3000
...
EOF
fi" "Создание .env файла"

# 5. Создание ecosystem.config.js если отсутствует
run_ssh_command "cd $APP_DIR && if [ ! -f ecosystem.config.js ]; then cat > ecosystem.config.js << 'EOF'
module.exports = {
  apps: [{
    name: 'nodeon-pyramid',
    script: 'server.js',
...
EOF
fi" "Создание ecosystem.config.js"
...
run_ssh_command "cd $APP_DIR && if [ -f index.html ] && [ ! -f public/index.html ]; then cp index.html public/; fi" "Копирование index.html в public"
